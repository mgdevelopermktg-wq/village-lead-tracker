/**
 * backfill-sources.js
 * One-time admin function — refreshes the source field for ALL leads from Spark.
 * Fixes leads where source is stored as '&mdash;' (HTML entity from spreadsheet
 * import) or is simply empty.
 *
 * Fetches marketing_source from Spark's /contacts/{id} detail endpoint and writes
 * the result back to Supabase regardless of the current value.
 *
 * Processes in batches of 20 (parallel) to stay within Netlify's 30s timeout.
 *
 * Usage:
 *   GET /.netlify/functions/backfill-sources?secret=<BACKFILL_SECRET>
...
function deriveSource(contact) {
  return (contact.marketing_source || '').trim();
}
...
export const handler = async (event) => {
  ...
  // Track processed spark_ids across repeated calls via ?done= param
  ...
  try {
    const { data: leads, error: fetchErr } = await supabase
      .from('leads')
      .select('id, spark_id, source, name');
    ...
    const results = await Promise.all(batch.map(async (lead) => {
      try {
        ...
        const source = deriveSource(contact);
        ...
        return { spark_id: lead.spark_id, name: lead.name, status: 'updated', source };
    ...
    console.error('Source backfill failed:', err);
